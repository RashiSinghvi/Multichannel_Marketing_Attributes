...
def viz_side(df):
    if(st.sidebar.checkbox("Revenue Exploration")):
        explore_revenue(df)
    elif(st.sidebar.checkbox("Revenue based on Browsers/Device/Operating System")):
        device_revenue(df)
    elif(st.sidebar.checkbox("Channel Groups and their Revenue")):
        channel_revenue(df)
    elif(st.sidebar.checkbox("Browsers used on Different Devices")):
        browser_devices(df)
    elif(st.sidebar.checkbox("Distribution of transaction Revenue of OS's")):
        os_revenue(df)
    elif(st.sidebar.checkbox("Distribution of transaction Revenue of Device Category")):
        device_dist(df)
    elif(st.sidebar.checkbox("Vistis/Revenue/Sellings by Day")):
        day_info(df)
    elif(st.sidebar.checkbox("Counting Visits Per Country")):
        country_visits(df)

def data_study_side(df):
    if(st.sidebar.checkbox("Show Dataset")):
        st.write(df.head(200))
    elif(st.sidebar.checkbox("Describe Each Column")):
       st.write(DataDesc(df))
    elif(st.sidebar.checkbox("Describe Columns With Null Values")):
        st.write(Null_Count(df))
        
        
def load_sidebar(df):
    st.sidebar.subheader("Multichannel marketing")
    
    add_selectbox = st.sidebar.selectbox('',('Choose an option','Data Study','Data Analysis'))
    if(add_selectbox == 'Data Study'):
        data_study_side(df)
    elif(add_selectbox == 'Data Analysis'):
        viz_side(df)
    st.sidebar.success('Git-Hub Link : https://github.com/Rakeshgehlot34/Multichannel_Marketing_Attributes')
...
def data_study(df):
    if(st.checkbox("Show Dataset")):
        st.write(df.head(200))
    elif(st.checkbox("Describe Each Column")):
       st.write(DataDesc(df))
    elif(st.checkbox("Describe Columns With Null Values")):
        st.write(Null_Count(df))
      
def main():
    st.title("Multichannel Marketing")
    data_file = st.file_uploader("Upload CSV",type=["csv"])
    load_sidebar(data_file)
    if data_file is not None:
        ...
        add_selectbox = st.selectbox('',('Choose an option','Data Study','Data Analysis','Revenue Prediction'))
        if(add_selectbox == 'Data Study'):
            data_study(data_file)
        elif(add_selectbox == 'Data Analysis'):
            viz(data_file)
        elif(add_selectbox=='Revenue Prediction'):
            st.write('Future Work : Working on ROI')
            st.write('using formula : (Total Revenue generated by channel/Investment)*100')
